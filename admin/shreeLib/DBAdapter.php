...
class DBAdapter {
    ...
    function getData($table, $field) {
        ...
        foreach ($field as $col) {
            if ($i == $count) {
                $string = $string . mysqli_real_escape_string($this->con,$col);
            } else {
                $string = $string . mysqli_real_escape_string($this->con,$col) . ",";
            }
            $i++;
    ...
    function getRow($table, $field, $clause) {
		...
		$id=mysqli_real_escape_string($this->con,$clause);
        $query = $text . $string . " from " . $table . " where " . $id;
//        echo '</br>';echo '</br>';echo '</br>';echo '</br>';
   //   echo $query;
    ...
    function getLastID($field, $table, $clause) {
        include 'dbconn.php';
		$id=mysqli_real_escape_string($this->con,$clause);
        $query = "select " . $field . " from " . $table . " where " . $id . " order by " . $field . " desc limit 1";
        //echo $query;
        $result = mysqli_query($this->con, $query);
        if ($result) {
            $data = 0;
